...
export const agentApiHelp = {
  ...
  agentSessionFileResource:
    "Export files, stage Project/SPICE imports, inspect one candidate Cell, or access simulation files. import-cell requires explicit source/target Cell IDs, replace-body/append mode, target revision and Project structure revision; it uses project.import plus existing edit scopes and one undoable commit. Compatible target interfaces and callers are preserved; conflicts reject atomically, keeping the candidate. open creates a new tab; request-approval is for whole-Project replacement. Artifact access never grants arbitrary host files.",
